...
export class ChannelMessagesService {
  ...
  async allMessages() {
    ...
    await this.messages$.subscribe((message: any) => {
      message.forEach((msg: any) => {
        msg.dateString = this.formatDateToYYYYMMDD(msg.dateString);
      });
  ...
  isMessageForNewDay(message: any, index: number): boolean {   
    ...
    // Hier wird das richtige Datumsformat verwendet
    const messageDate = message.dateString;
    const previousMessageDate = previousMessage.dateString;
