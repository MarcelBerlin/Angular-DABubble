...
export class MessageInputThreadService {
  private myVariableSubjectThread = new BehaviorSubject<boolean>(false);
  myVariableThread$: Observable<boolean> = this.myVariableSubjectThread.asObservable();
  inputLinks: any[] = [];
  ...
  setMyVariable(newValue: boolean): void {
    this.myVariableSubjectThread.next(newValue);
  }
